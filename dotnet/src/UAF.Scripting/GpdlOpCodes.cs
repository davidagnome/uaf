...
// Transcribed verbatim from src/Shared/GPDLOpCodes.h. The values are load-bearing: they are baked
// into every compiled talk.bin and into the script blobs stored inside design files, so
// renumbering silently breaks every existing design. The gaps, and the DEPRECATED_/NOT_USED_
// names, are in the original and are kept -- the numbers around them must not shift.
//
// Do not confuse this with src/Shared/GPDLOpCodes.h.bak, which carries ~140 extra SUBOP names.
// The .bak is not what the live build compiles against; several GPDLexec.cpp cases referencing
// those extra names sit inside /* ... */ blocks precisely because the live enum lacks them
// (for example SUBOP_SET_SPELL_SA at GPDLexec.cpp:2785).

/// <summary>
/// The primary opcode, carried in the <b>top byte</b> of each 32-bit code word. The low 24 bits
/// are an address, a sub-opcode, or a count depending on the primary opcode.
/// </summary>
public enum BinOp
{
    BINOP_ILLEGAL = 0,
...
/// <summary>
/// The sub-opcode carried in the low 24 bits when the primary opcode is
/// <see cref="BinOp.BINOP_SUBOP"/>. This is the entire system-function space.
/// </summary>
public enum SubOp
{
    SUBOP_ILLEGAL = 0x000,
    ...
    SUBOP_DEPRECATED_SET_LEVEL_STATS_ASL = 0x21,
    SUBOP_DEPRECATED_DELETE_LEVEL_STATS_ASL = 0x22,
...
/// <summary>
/// Opcodes for the <c>RDRCOMP</c>/<c>RDREXEC</c> pair -- a separate, byte-coded expression
/// language for "runtime data references", unrelated to the GPDL word-coded VM. Declared for
/// completeness; the RDR compiler and interpreter are <b>not</b> ported (GPDLcomp.cpp:4131,
/// GPDLexec.cpp:8249).
/// </summary>
public enum COp
{
...
/// <summary>
/// The <c>CTX_*</c> bit flags from GPDLOpCodes.h. A system function declares the contexts it
/// needs; <c>GPDLCOMP::compileTypedSystemFunctionCall</c> (GPDLcomp.cpp:2809) rejects a call whose
/// required contexts are not all present in the hook's available set.
/// </summary>
[Flags]
public enum GpdlContexts : uint
{
    None = 0,
    CTX_Combatant = 0x00000001,
    CTX_Trait = 0x00000002,
    CTX_Target = 0x00000004,
    CTX_Character = 0x00000008,
    CTX_Attacker = 0x00000010,
    CTX_Item = 0x00000020,
    CTX_Class = 0x00000040,
    CTX_Race = 0x00000080,
    CTX_Spell = 0x00000100,
    CTX_BaseClass = 0x00000200,
    CTX_Myself = 0x00000400,
    CTX_SpellGroup = 0x00000800,
    CTX_MonsterType = 0x00001000,
    CTX_Ability = 0x00002000,
}

/// <summary>Composite values that the C++ open-codes in both halves of the subsystem.</summary>
public static class GpdlCode
{
    /// <summary>
    /// <c>BINOP_SUBOP &lt;&lt; 24</c> -- <c>SHIFTED_BINOP</c> in GPDLOpCodes.h. Every system
    /// function reference is this OR'd with a <see cref="SubOp"/>.
    /// </summary>
    public const uint ShiftedSubOp = (uint)BinOp.BINOP_SUBOP << 24;

    /// <summary>Extracts the primary opcode (top byte) from a code word.</summary>
    public static BinOp OpOf(uint word) => (BinOp)((word >> 24) & 0xff);

    /// <summary>Extracts the low 24 bits: an address, a sub-opcode, or a count.</summary>
    public static uint OperandOf(uint word) => word & 0xffffff;

    /// <summary>
    /// Bit 23 of a <see cref="BinOp.BINOP_ReferenceGLOBAL"/> operand means "store" rather than
    /// "fetch" (GPDLcomp.cpp:2426, GPDLexec.cpp:2315). The remaining 23 bits are the global index.
    /// </summary>
    public const uint GlobalStoreBit = 0x800000;

    /// <summary>
    /// Frame-pointer offsets are stored as a 24-bit two's-complement value, so locals get negative
    /// offsets. GPDLexec.cpp:2362 sign-extends with
    /// <c>subop -= (subop &amp; 0x800000) &lt;&lt; 1</c>.
    /// </summary>
    public static int SignExtend24(uint operand) =>
        (int)operand - (int)((operand & 0x800000) << 1);
}

...
public class Tier3LzwTests
{
    ...
    public static TheoryData<string, double> Fixtures => new()
    {
        ...
        // Generated by the Oracle workflow itself: DefaultDesign re-saved at PRODUCT_VER.
        // Uniquely valuable because the oracle ALSO dumped it, so C# and C++ readings of
        // the very same bytes can be compared directly.
        { Path.Combine("ci-tier3", "Data"), 5.29 },
    };
    ...
    public void Decompressed_records_yield_readable_item_names(string dataDir, double expectedVersion)
    {
        ...
        // spellID: ver >= 0.999647 (Items.cpp:2761), true for every fixture here.
        //
        // It is read as a STRING, not an int -- `SPELL_ID` derives from CString
        // (Externs.h:1324), so `ar >> spellID` goes through CAR's string path. The name reads
        // like an identifier and the field sits among integers, which is exactly why this was
        // mis-modelled twice: first skipped entirely, then read as one int. Both produced
        // printable output, because a one-field shift in an interned-string stream yields
        ...
        // Readable names are the decisive evidence: LZW output is either exactly right or it
        // is noise, and noise is not printable ASCII.
        Assert.All(uniqueName, ch => Assert.InRange(ch, ' ', '~'));
        Assert.All(idName, ch => Assert.InRange(ch, ' ', '~'));

        // Which of the two carries content varies by design: DefaultDesign-derived fixtures open
        // with a real item ("Arrow", both names set), while the community designs open with a
        // "template" placeholder whose idName is blank. Requiring a specific one would be
        // asserting a property of the fixture rather than of the decoder.
        Assert.False(string.IsNullOrEmpty(uniqueName) && string.IsNullOrEmpty(idName),
            "first record has neither name - the compressed stream was not decoded correctly");
    ...
    /// <summary>
    /// Diffs the C# tier-3 reading against the C++ oracle's reading of <b>the same bytes</b>.
    /// </summary>
    /// <remarks>
    /// The Oracle workflow generates this design (DefaultDesign re-saved at 5.29) and dumps it in
    /// the same run, so <c>Tier3Design.json</c> is the reference's own reading of exactly these
    /// files. Nothing else in the suite compares both implementations on identical tier-3 input.
    /// </remarks>
    [Fact]
    public void Tier3_item_names_agree_with_the_oracle_on_identical_bytes()
    {
        string dir = Path.Combine(RepoRoot(), "reference", "ci-tier3");
        string dump = Path.Combine(dir, "Tier3Design.json");
        string items = Path.Combine(dir, "Data", "items.dat");
        if (!File.Exists(dump) || !File.Exists(items)) { return; }

        using var doc = System.Text.Json.JsonDocument.Parse(File.ReadAllText(dump));
        var expected = doc.RootElement.GetProperty("itemNames");

        using var fs = File.OpenRead(items);
        var header = DesignFileHeader.Read(fs, DesignFileKind.Database);
        Assert.Equal(ArchiveTier.CompressedCar, header.Tier);
        ...
        car.ReadInt32();    // preSpellNameKey  (ver >= VersionSaveIDs)   -- an int
        car.ReadString();   // spellID          (ver >= 0.999647)      -- a CString subclass
        string uniqueName = ArchiveStringConventions.Decode(car.ReadString());
        string idName = ArchiveStringConventions.Decode(car.ReadString());

        Assert.Equal(expected[0].GetProperty("uniqueName").GetString(), uniqueName);
        Assert.Equal(expected[0].GetProperty("idName").GetString(), idName);
    }

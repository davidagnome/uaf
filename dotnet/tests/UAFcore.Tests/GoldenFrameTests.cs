...
public class GoldenFrameTests
{
    ...
    /// <summary>
    /// The checked-in golden under <c>dotnet/tests/</c>, or null when the source tree is not
    /// reachable from where the tests are running.
    /// </summary>
    private static string? SourceGoldenPath()
    {
        ...
        string assets = Path.Combine(dir.FullName, "dotnet", "tests", "UAFcore.Tests", "Assets");
        return Directory.Exists(assets) ? Path.Combine(assets, GoldenFile) : null;
    }
    ...
    public void The_viewport_matches_its_golden_hashes()
    {
        ...
        {
            // Written to the SOURCE tree, not to AppContext.BaseDirectory. Updating the build
            // output instead looks like it worked -- the run goes green, because the file the
            // comparison reads is the one just overwritten -- and then the next clean build
            // copies the unchanged checked-in file back over it and the failure returns. That
            // happened; hence the two destinations.
            string[] lines =
            ...
            foreach (string destination in new[] { path, SourceGoldenPath() }.OfType<string>())
            {

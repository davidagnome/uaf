...
so all three shipped databases are reproduced **byte for byte**, and a saved character is written
as a whole file. Its round-trip exit criterion still needs the event bodies, `GLOBAL_STATS`, levels
and the rest of the save games.
...
**2,657 tests, green on macOS, Linux and Windows; both CI workflows green.**
...
for byte, and a `.chr` file is written whole. What is left is the 44 event storing branches, then
`GLOBAL_STATS` and `LEVEL` — which both need them — and the rest of the savegames. Phase 5's exit
(the editor saves a design the C++ editor can load) depends entirely on those.
...
2. **The rest of the archive writer.** The byte layer, every shared leaf, the `CAR` write path and
   **four of the six record types** are done — monsters, items and spells each reproducing
   ...
   The `.chr` half of the savegames is already done (§the first whole file outside a database).
   What remains, in order — **and the order changed once the `GLOBAL_STATS` storing branch was
   actually read**:
   - **The 44 event storing branches.** An earlier revision of this list put `GLOBAL_STATS` next.
     That was wrong: its storing branch ends with `eventData.Serialize(car, version)`
     (`GlobalData.cpp:4556`), the design's global event list — so the event bodies are a
     *prerequisite* of `GLOBAL_STATS`, not a later concern. `LEVEL` needs them too, and far more of
     them. They are the shared blocker for both remaining records and should be taken first.
     Twenty-six of the 44 types already execute in the engine, so their shapes are well understood;
...
|---|---|---|
| **`ArchiveWriter`** | The byte layer, every shared leaf, `MONSTER_DATA`, `ITEM_DATA`, `SPELL_DATA`, `CHARACTER`, `.chr` files, the whole `CAR` write path and the writer cursor are done — and **three shipped databases are reproduced byte for byte**. What remains is the 44 event bodies (which `GLOBAL_STATS` and `LEVEL` both need), then those two and the savegames. Phase 1's round-trip exit criterion is met for the three databases; **Phase 5 cannot begin** until an editor can save a whole design | Large |

...
Scaffolded so far: `UAF.Common`, `UAF.Serialization`, `UAF.Data`, `UAF.Media`, `UAF.Media.Sdl`,
`UAF.Serialization.Tests`, `UAF.Media.Tests`, both workflows, and the version generator. The rest are
created as their phase begins — empty projects are just restore risk.
...
- **Licensing:** LGPL builds are the default and sit comfortably under this project's GPL v2
  ...
  **Correction (Phase 3):** this considered FFmpeg's own licence but not the C# bindings'. Every
  binding on NuGet — `FFmpeg.AutoGen`, `Sdcb.FFmpeg`, and therefore `FFMediaToolkit`, which depends
  on the first — is **LGPL v3**, so linking one makes the combined work GPL v3. Still permitted, but
  it removes the GPL v2 option for any build that includes it, which is a second reason to keep video
  in its own optional assembly. See §7 Phase 3 Progress.

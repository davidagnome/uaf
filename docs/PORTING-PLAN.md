...
shared leaf and **three whole record types**, monsters, items and spells, which round-trip every
record in the corpus — and **the `CAR` write path now exists**, both halves of it, so all three of
those databases are reproduced **byte for byte** from a shipped design. Its round-trip exit
criterion still needs characters, levels and save games.
...
**2,625 tests, green on macOS, Linux and Windows; both CI workflows green.**
...
| Writers | Byte layer, `CAR` write path and every shared leaf done. **`MONSTER_DATA`, `ITEM_DATA` and `SPELL_DATA` write**, and all three of `ci-tier3`'s databases are reproduced byte for byte. `CHARACTER`, `LEVEL` and the savegames do not |
...
`ITEM_DATA` and `SPELL_DATA`, which reproduce `ci-tier3`'s three databases byte for byte. What is
left is the storing branch of `CHARACTER`, `LEVEL` and the savegames. Phase 5's exit (the editor
saves a design the C++ editor can load) depends entirely on those.
...
2. **The rest of the archive writer.** The byte layer, every shared leaf, the `CAR` write path and
   **three of the six record types** are done — monsters, items and spells, each reproducing
   `ci-tier3`'s database byte for byte (§the archive writer's first layers, and the three
   record-type sections). What is missing is a writer per remaining record type, mirroring each
   reader. It is still the largest structural gap in the port: it gates Phase 1's round-trip exit
   criterion, save games, and Phase 5 entirely. In order:
   - **`CHARACTER`** — the format's largest record by field count, and the one the savegames need.
     `BaseclassRecordReader` is its neighbour and `baseclass.dat` still has no writer either.
   - **`LEVEL` / `ZONE`**, which is what an editor actually edits: the grid, the events, the zones
     and the wall/background sets. The event bodies are 44 separate storing branches under it.
   - **The savegames** — `PARTY`, `.CHAR` and cell contents. Phase 1's stated exit criterion.
...
|---|---|---|
| **`ArchiveWriter`** | The byte layer, every shared leaf, `MONSTER_DATA`, `ITEM_DATA`, `SPELL_DATA`, the whole `CAR` write path and the writer cursor are done — and **three shipped databases are reproduced byte for byte**. What remains is characters, levels and save games. Phase 1's round-trip exit criterion is met for the three databases; **Phase 5 cannot begin** until an editor can save a whole design | Large |

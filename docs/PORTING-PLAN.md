...
| ~~**Global script hooks**~~ | **Done.** `SpecabScripts` runs a record's own abilities with the reference's callbacks, and `CanCastSpells` and `FIX_CHARACTER` go through it. `CombatPlacement` is done, and `PartyArrangement` / `PartyOrigin<direction>` are wired through `PartyArrangementScript` into `CombatSetup` — a design's `Global_Combat` hooks now supply the formation table and a per-direction origin offset | — |
| **`GenerateOutdoorCombatMap`** | Outdoor encounters have no map, so wilderness combat uses the indoor generator. Bigger than this row used to claim: it is not "same three-pass shape" — the outdoor path places random tiles by `WildernessTileDensity` and then expands the 35 wilderness group types (`DBL_HIGH_*` / `SGL_HIGH_*`, 100–134), which `CombatTerrainExpander` does not handle (they fall through). The obstacle semantics of those tile indices are also not in the port's obstacle-based combat model, and no shipped design runs an outdoor encounter to validate against | Medium |

...
so all three shipped databases are reproduced **byte for byte**. Its round-trip exit criterion
still needs levels and save games.
...
**2,651 tests, green on macOS, Linux and Windows; both CI workflows green.**
...
for byte, and the fourth has no shipped file of its own to compare against. What is left is the
storing branch of `LEVEL`, the savegames, and `GLOBAL_STATS` — which a character list sits inside.
...
2. **The rest of the archive writer.** The byte layer, every shared leaf, the `CAR` write path and
   **four of the six record types** are done — monsters, items and spells each reproducing
   ...
   - **`GLOBAL_STATS`**, the record a character list sits inside — and therefore the thing that
     turns the character writer into a `game.dat` the reference can open. `GlobalStatsReader` is
     335 lines and the tail readers another 425, so it is the widest of the three rather than the
     deepest.
   ...
   - **The savegames** — `PARTY`, `.CHAR` and cell contents. Phase 1's stated exit criterion, and
     the `.CHAR` half is a `CHARACTER` with a header, so it is nearly free now.
...
|---|---|---|
| **`ArchiveWriter`** | The byte layer, every shared leaf, `MONSTER_DATA`, `ITEM_DATA`, `SPELL_DATA`, `CHARACTER`, the whole `CAR` write path and the writer cursor are done — and **three shipped databases are reproduced byte for byte**. What remains is `GLOBAL_STATS`, levels and save games. Phase 1's round-trip exit criterion is met for the three databases; **Phase 5 cannot begin** until an editor can save a whole design | Large |

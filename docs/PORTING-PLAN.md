...
| **GPDL reference bytecode** | `oracle/golden/gpdl/` holds 4 scripts and **0 `.bin` goldens**, so `GpdlOracleDiffTests` returns early and Phase 2's exit criterion is undemonstrated. **The workflow step now exists** — `Compile GPDL goldens` in `oracle-cpp.yml` compiles all four with the reference `GPDLcomp.exe`, refuses to publish a partial set, and uploads a `gpdl-goldens` artifact; a later step hashes fresh output against the committed goldens to catch reference drift. What is left is one green Oracle run and committing the artifact — the workflow has `contents: read` and deliberately does not self-commit | Small |
